...
export async function generate_message(params: GenerateMessageParams): Promise<string> {
  ...
  if (!llm_response.success) {
    ...
    return add_footer("Update with changes from staging area", config.llm_model)
  }
  ...
  if (!validation_result.valid) {
    ...
    if (validation_result.suggested_fix) {
      ...
      return add_footer(validation_result.suggested_fix, config.llm_model)
    }
  ...
  return add_footer(llm_response.text, config.llm_model)
}
...
function add_footer(message: string, model: string): string {
  const timestamp = lib_datetime.format_iso(new Date())
  const footer = `\n\nGenerated by: ${lib_package_details.PROGRAM_NAME} v${lib_package_details.PROGRAM_VERSION} (${timestamp}) using ${model}`
